...
public class Coach implements ControllerCoach {
    ...
    private byte[] sayArray = new byte[25];
    ...
    public void infoSeePlayerOwn(int number, boolean goalie, double x, double y, double deltaX, double deltaY,
                                 double bodyAngle, double neckAngle) {
        System.out.println("see player own: "+number);
        // xy are put in the array next to each other
        sayArray[number*2] = (byte) Math.round(x); // create byte of x position and put in the array positio 2*number(of player)
        sayArray[number*2+1] = (byte) Math.round(y); // create byte of x position and put in the array 1+2*number(of player)
        for (int i=0; i<22; i++){
            System.out.println(sayArray[i]);
        }
        String s = Base64.encode(sayArray);
        //coach.say(s);
    }
